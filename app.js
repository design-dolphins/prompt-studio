...
const exampleRequests = {
  proposal: "webサイトの企画提案書の構成を作るためのプロンプトを作って",
  html: "企画書の内容からLPのHTMLモックを作るためのプロンプトを作って",
  wireframe: "添付資料をもとにWebサイトのワイヤーフレームを作るプロンプトを作って",
  policy: "自社の新しい社内制度を整理するためのプロンプトを作って",
  "ui-review": "サイトのスクリーンショットを見てUI/UXレビューをしてもらうプロンプトを作って",
...
const roleMap = {
  ...
  html: "Webフロントエンドエンジニア兼UIモックアップ生成AI",
...
const templateConfigs = {
  ...
  html: {
    goals: [
      "企画書・構成書を視覚的にレビュー可能なHTMLモックへ変換する",
      "ワイヤーフレーム以上・本デザイン未満の粒度で、合意形成を加速させる",
      "実装チームに意図が正確に伝わる構造・コメントを備える",
    ],
    constraints: [
      "HTML + CSSを1つのコードブロックで返す",
      "`<meta charset=\"UTF-8\">` を必ず含め、文字化けを防ぐ",
      "CSSは `<style>` 内、またはインラインで完結させる",
      "画像はプレースホルダーを使用する",
      "文言は差し替えやすい仮文でよい",
      "グレースケール基調で、装飾過多にしない",
      "セクションごとにコメントで区切る",
    ...
    materials: ["セクション別テキスト", "使用アイコン/ライブラリ", "既存ページURL", "想定画面幅（SP優先 / PC中心）", "参考デザイン / 参考URL"],
    output: ["単一HTMLモック", "FV", "課題", "解決", "実績", "CTA", "必要に応じた補足説明"],
    options: ["複数デザイン案", "スマホ版重視", "CTA改善", "コピー改善", "UIコンポーネント単位で生成"],
  },
  wireframe: {
    goals: [
